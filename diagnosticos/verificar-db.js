...
(async () => {
  ...
  if (exportCSV) {
    ...
    fs.writeFileSync(path.resolve("/app", "diagnostico-verificar-db.csv"), csv);
  }

  if (exportJSON) {
    fs.writeFileSync(path.resolve("/app", "diagnostico-verificar-db.json"), JSON.stringify(output, null, 2));
  }
})();
}
main()
