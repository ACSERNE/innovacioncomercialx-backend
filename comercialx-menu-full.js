...
async function crearProducto() {
  ...
  try {
    ...
    console.log('❌ Error al crear producto:', err.response?.data?.message || err.message);
  }
}
...
async function flujoCaja() {
  try {
    ...
    console.log('❌ Error al consultar flujo de caja:', err.response?.data?.message || err.message);
  }
}

// Placeholder: puedes agregar aquí funciones reales para editar, eliminar, reportes, etc.
async function publicarProducto() { console.log('🛒 Publicar producto (pendiente)'); }
async function editarProducto() { console.log('✏️ Editar producto (pendiente)'); }
async function eliminarProducto() { console.log('❌ Eliminar producto (pendiente)'); }
async function verInventario() { console.log('📦 Inventario (pendiente)'); }
async function stockProducto() { console.log('📦 Stock por producto (pendiente)'); }
async function estadisticas() { console.log('📈 Estadísticas (pendiente)'); }
async function buscarProducto() { console.log('🔍 Buscar producto (pendiente)'); }
async function reportesFiltrados() { console.log('📊 Reportes filtrados (pendiente)'); }
async function exportarAuditoria() { console.log('📝 Exportar auditoría Markdown (pendiente)'); }
async function filtrarAuditoria() { console.log('🧾 Filtrar auditoría (pendiente)'); }
async function verFirma() { console.log('🔐 Firma local de sesión (pendiente)'); }
async function exportacionesCockpit() { console.log('📊 Exportaciones cockpitizadas (pendiente)'); }
